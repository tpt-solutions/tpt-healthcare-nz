...
// Deployable independently to serve hospital cardiac units and outpatient clinics.
package api
...
func (s *Server) buildRoutes() *http.ServeMux {
	...
	// TODO (Milestone 10b): implement cardiology handlers
	mux.Handle("GET /api/v1/cardiology/orders", chain(http.HandlerFunc(notImplemented)))
	mux.Handle("POST /api/v1/cardiology/orders", chain(http.HandlerFunc(notImplemented)))
	mux.Handle("GET /api/v1/cardiology/orders/{id}", chain(http.HandlerFunc(notImplemented)))
	mux.Handle("POST /api/v1/cardiology/orders/{id}/result", chain(http.HandlerFunc(notImplemented)))
	mux.Handle("GET /api/v1/cardiology/cath-lab", chain(http.HandlerFunc(notImplemented)))
	mux.Handle("POST /api/v1/cardiology/cath-lab", chain(http.HandlerFunc(notImplemented)))
	mux.Handle("GET /api/v1/cardiology/cath-lab/{id}", chain(http.HandlerFunc(notImplemented)))
	mux.Handle("POST /api/v1/cardiology/cath-lab/{id}/start", chain(http.HandlerFunc(notImplemented)))
	mux.Handle("POST /api/v1/cardiology/cath-lab/{id}/complete", chain(http.HandlerFunc(notImplemented)))

	return mux
}

func (s *Server) handleHealth(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, http.StatusOK, map[string]string{"status": "ok", "service": "tpt-cardiology", "time": time.Now().UTC().Format(time.RFC3339)})
}
