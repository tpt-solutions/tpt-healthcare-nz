-- 001_methadone_programme.sql
-- Opioid Substitution Therapy programme tables for NZ methadone / buprenorphine services.
-- Enforces extra_sensitive flag consistent with mental-health module pattern.
...
CREATE INDEX idx_addiction_programmes_patient ON addiction_programmes(tenant_id, patient_nhi);
CREATE INDEX idx_addiction_programmes_clinician ON addiction_programmes(tenant_id, clinician_id);
CREATE INDEX idx_addiction_programmes_phase ON addiction_programmes(tenant_id, phase) WHERE phase <> 'discharged';
...
-- Dose records — every supervised or take-home dose administered.
...
CREATE INDEX idx_methadone_doses_programme ON methadone_doses(tenant_id, programme_id, administered_at DESC);

ALTER TABLE methadone_doses ENABLE ROW LEVEL SECURITY;
CREATE POLICY methadone_doses_tenant_only ON methadone_doses
    USING (tenant_id = current_setting('app.current_tenant_id')::UUID);

-- Take-home approval history.
...
CREATE INDEX idx_take_home_programme ON methadone_take_home_approvals(tenant_id, programme_id, approved_at DESC);
...
CREATE INDEX idx_urine_screens_programme ON urine_screens(tenant_id, programme_id, collected_at DESC);
