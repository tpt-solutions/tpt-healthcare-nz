-- OST prescriptions and dose adjustments (methadone, buprenorphine, Suboxone).
-- Controlled drug: every write audited via application layer per HIPC Rule 10/11.
...
CREATE INDEX IF NOT EXISTS idx_ost_prescriptions_patient    ON ost_prescriptions (tenant_id, patient_nhi, status);
CREATE INDEX IF NOT EXISTS idx_ost_prescriptions_programme  ON ost_prescriptions (tenant_id, programme_id);
...
CREATE INDEX IF NOT EXISTS idx_ost_adjustments_prescription ON ost_dose_adjustments (tenant_id, prescription_id, adjusted_at DESC);
