// Package main is the entrypoint for the tpt-doctor CLI.
...
import (
	...
	"github.com/PhillipC05/tpt-healthcare/modules/tpt-doctor/api"
...
var rootCmd = &cobra.Command{
	Use:   "tpt-doctor",
	Short: "tpt-doctor — GP clinical management service for NZ",
	Long: `tpt-doctor is the general practice clinical management service for
the tpt-healthcare NZ platform. It exposes a FHIR R5-aligned REST API
for patient management, appointments, encounters, prescriptions, and ACC claims.`,
}

func init() {
	cobra.OnInitialize(initConfig)

	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default: tpt-doctor.yaml in current directory or /etc/tpt)")
	...
	serveCmd.Flags().IntVar(&port, "port", 8082, "port to listen on")
...
func initConfig() {
	if cfgFile != "" {
		...
		viper.SetConfigName("tpt-doctor")
...
var serveCmd = &cobra.Command{
	Use:   "serve",
	Short: "Start the tpt-doctor HTTP server",
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		go func() {
			logger.Info("tpt-doctor server starting", slog.String("addr", addr))
...
var migrateCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if dbURL == "" {
			return fmt.Errorf("database.url is required (set TPT_DOCTOR_DATABASE_URL)")
		}
...
var validateCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		logger.Info("validating tpt-doctor configuration")
