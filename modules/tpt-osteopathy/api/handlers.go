...
import (
	...
	"github.com/PhillipC05/tpt-healthcare/modules/tpt-osteopathy/internal/assessment"
	"github.com/PhillipC05/tpt-healthcare/modules/tpt-osteopathy/internal/treatment"
...
func (s *Server) handleListAssessments(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, []assessment.Assessment{})
}
...
func (s *Server) handleCreateAssessment(w http.ResponseWriter, r *http.Request) {
	...
	a.ClinicianID = principal.PractitionerID
	now := time.Now().UnixMilli()
	a.CreatedAt = now
	a.UpdatedAt = now
	s.recordEvent(r, principal, "create", "OsteopathyAssessment", a.ID, a.PatientNHI)
...
func (s *Server) handleGetAssessment(w http.ResponseWriter, r *http.Request) {
	...
	s.recordEvent(r, principal, "read", "OsteopathyAssessment", assessmentID, patientNHI)
	writeJSON(w, http.StatusNotFound, apiError{Code: "NOT_FOUND", Message: fmt.Sprintf("assessment %s not found", assessmentID)})
}
...
func (s *Server) handleUpdateAssessment(w http.ResponseWriter, r *http.Request) {
	...
	a.ID = assessmentID
	a.PatientNHI = patientNHI
	a.ClinicianID = principal.PractitionerID
	a.UpdatedAt = time.Now().UnixMilli()
	s.recordEvent(r, principal, "update", "OsteopathyAssessment", assessmentID, patientNHI)
...
func (s *Server) handleListTreatments(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, []treatment.Treatment{})
}
...
func (s *Server) handleCreateTreatment(w http.ResponseWriter, r *http.Request) {
	...
	r.Body = http.MaxBytesReader(w, r.Body, 1<<20)
	var t treatment.Treatment
	if err := decodeJSON(r, &t); err != nil {
		writeJSON(w, http.StatusBadRequest, apiError{Code: "INVALID_JSON", Message: err.Error()})
		return
	}
	if !nhi.ValidateNHI(t.PatientNHI) {
		writeJSON(w, http.StatusBadRequest, apiError{Code: "INVALID_NHI", Message: "patientNhi format is invalid"})
		return
	}
	t.ID = uuid.New().String()
	t.ClinicianID = principal.PractitionerID
	now := time.Now().UnixMilli()
	t.CreatedAt = now
	t.UpdatedAt = now
	s.recordEvent(r, principal, "create", "OsteopathyTreatment", t.ID, t.PatientNHI)
...
func (s *Server) handleGetTreatment(w http.ResponseWriter, r *http.Request) {
	...
	s.recordEvent(r, principal, "read", "OsteopathyTreatment", treatmentID, patientNHI)
	writeJSON(w, http.StatusNotFound, apiError{Code: "NOT_FOUND", Message: fmt.Sprintf("treatment %s not found", treatmentID)})
}
...
func (s *Server) handleUpdateTreatment(w http.ResponseWriter, r *http.Request) {
	...
	t.ID = treatmentID
	t.PatientNHI = patientNHI
	t.ClinicianID = principal.PractitionerID
	t.UpdatedAt = time.Now().UnixMilli()
	s.recordEvent(r, principal, "update", "OsteopathyTreatment", treatmentID, patientNHI)
