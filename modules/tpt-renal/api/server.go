...
func (s *Server) buildRoutes() *http.ServeMux {
	...
	// TODO (Milestone 10b): implement renal handlers
	mux.Handle("GET /api/v1/renal/patients", chain(http.HandlerFunc(notImplemented)))
	mux.Handle("POST /api/v1/renal/patients", chain(http.HandlerFunc(notImplemented)))
	mux.Handle("GET /api/v1/renal/patients/{id}", chain(http.HandlerFunc(notImplemented)))
	mux.Handle("GET /api/v1/renal/patients/{id}/sessions", chain(http.HandlerFunc(notImplemented)))
	mux.Handle("POST /api/v1/renal/patients/{id}/sessions", chain(http.HandlerFunc(notImplemented)))
	mux.Handle("PUT /api/v1/renal/patients/{id}/sessions/{sessionId}", chain(http.HandlerFunc(notImplemented)))
	mux.Handle("POST /api/v1/renal/patients/{id}/sessions/{sessionId}/complete", chain(http.HandlerFunc(notImplemented)))

	return mux
}

func (s *Server) handleHealth(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, http.StatusOK, map[string]string{"status": "ok", "service": "tpt-renal", "time": time.Now().UTC().Format(time.RFC3339)})
}
